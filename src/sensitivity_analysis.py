...
def load_external_data() -> Tuple[Dict, Dict, Dict]:
    ...
    toxref_path = Path(EXTERNAL_DIR) / "toxref.yml"
    toxref = {}
    ...
    kp_path = Path(EXTERNAL_DIR) / "dermal_water_kp.yml"
    kp_table = {}
    if kp_path.exists():
        try:
    ...
    # Load PHREEQC speciation data if available (check results_dir passed in or default)
    speciation_data = {}
    # Try common results locations
    bio_path = Path(PROJECT_ROOT) / "results" / "table_bioavailable_concentrations.csv"
    frac_path = Path(PROJECT_ROOT) / "results" / "table_species_fractions.csv"
...
Examples:
  ...
  python sensitivity_analysis.py --input my_data.csv --output-dir my_results
    ...
    parser.add_argument('--input-units', choices=['ug/L', 'mg/L', 'ppb', 'ppm'],
                       default='ug/L',
    ...
    if args.input_units in ['mg/L', 'ppm']:
        logger.info("Converting concentrations from mg/L to µg/L (multiply by 1000)")
        for col in conc_df.columns:
            if col.startswith('C_') or col in DEFAULT_METALS:
                conc_df[col] = conc_df[col] * 1000
