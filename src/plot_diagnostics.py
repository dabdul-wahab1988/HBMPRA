...
def plot_convergence_summaries(trace_file, out_png=None):
    ...
    try:
        ...
        rhat_means = []
        ess_vals = []
        # idata.posterior may be an xarray Dataset; guard attribute access
        posterior = getattr(idata, "posterior", None)
        if posterior is None:
            # In some environments the attribute may not be present; avoid raising attribute errors
            var_names = []
        else:
            for var in posterior.data_vars:
                var_names.append(var)
                # rhat[var] can be an xarray DataArray, numpy array, or a scalar float;
                # normalize to a numpy array (or array-like) before taking the mean.
                rhat_val = np.nan
                if rhat is not None and var in rhat:
                    try:
                        rv = rhat[var]
                        arr = getattr(rv, "values", np.asarray(rv))
                        rhat_val = float(np.nanmean(arr))
                    except Exception:
                        rhat_val = np.nan
                ...
                if ess is not None and var in ess:
                    try:
                        ev = ess[var]
                        arr_e = getattr(ev, "values", np.asarray(ev))
                        ess_val = float(np.nanmean(arr_e))
                    except Exception:
                        ess_val = np.nan
...
def plot_prior_vs_posterior_vars(model, trace, vars_list, output_dir, prior_samples=2000):
    ...
    for var in vars_list:
        ...
        # prior if available
        prior_group = getattr(idata_prior, "prior", None) if idata_prior is not None else None
        if prior_group is not None and var in getattr(prior_group, "data_vars", []):
...
def plot_prior_posterior_panel(model, trace, vars_list, output_dir, prior_samples=2000):
    """Create a 3x3 panel plot (up to 9 vars) showing prior vs posterior densities with MathTeX labels."""
    if not vars_list:
        print("No variables provided for panel plot.")
        return
    # limit to 9 variables for 3x3 grid
    vars_panel = vars_list[:9]
    # sample priors
    with model:
        try:
    ...
    for i, var in enumerate(vars_panel):
        ...
        # Position legend at upper right with increased font size (40% larger)
    ...
    for j in range(len(vars_panel), 9):
...
def plot_posterior_distributions(trace, output_dir):
    ...
    colors = plt.get_cmap('tab10').colors  # Use a colormap for consistent colors
...
def plot_combined_prior_posterior(model, trace, output_dir, prior_samples=2000):
    ...
    for i, param in enumerate(params):
        # Prior density (if available)
        prior_group = getattr(idata_prior, "prior", None) if idata_prior is not None else None
        if prior_group is not None and param in getattr(prior_group, "data_vars", []):
            prior_vals = prior_group[param].values.flatten()
