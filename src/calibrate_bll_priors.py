...
def main():
    ...
    parser.add_argument("--chemistry", required=True, help="Path to measured concentrations CSV with C_Pb or Pb column.")
    ...
    parser.add_argument("--input-units", choices=['ug/L', 'mg/L'], default='ug/L',
                        help="Concentration units in input file (default: ug/L). If mg/L, will convert to µg/L for calibration.")
    ...
    # The BLL engines (edi_from_conc_ugL) expect concentrations in µg/L
    input_units_original = args.input_units
    if args.input_units == 'mg/L':
        print(f"Converting Pb concentrations from mg/L to µg/L (multiply by 1000)")
        cpb = cpb * 1000.0  # mg/L -> µg/L
        print(f"  Pb range after conversion: {cpb.min():.2f} - {cpb.max():.2f} µg/L")
    ...
    priors['engine_metadata'] = {
        ...
        # Calibration tracking fields - used by hbmpra_optimized.py
        'calibration_date': np.datetime_as_string(np.datetime64('now'), unit='s'),
        'calibration_source': os.path.basename(args.chemistry),
        'date': np.datetime_as_string(np.datetime64('now'), unit='s'),  # legacy
        'input_units_original': input_units_original,
        'calibration_units': 'µg/L',  # Always calibrated in µg/L internally
